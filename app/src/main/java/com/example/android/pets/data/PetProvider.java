...
public class PetProvider extends ContentProvider {
    ...
    static {
        // Uri Matcher for entire pets table
        sUriMatcher.addURI(PetContract.CONTENT_AUTHORITY, PetContract.PATH_PETS, PETS);
        // Uri Matcher for one row in the table
    ...
    private Uri insertPet(Uri uri, ContentValues values) {
        ...
        // Check that the weight is not null, above 0
        ...
        // Insert the new pet pet with the given values
        long id = database.insert(PetEntry.TABLE_NAME, null, values);

        // If the id = -1, then the insertion failed. Log and error and return null.
        if (id == -1) {
    ...
    private int updatePet(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // If one or more rows were updated, then notify all listeners that the data has.
        if (rowsUpdated != 0) {
            getContext().getContentResolver().notifyChange(uri, null);
        }
